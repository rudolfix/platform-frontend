...
export type TDocumentExtension = "doc" | "pdf";

interface IProps {
  extension: TDocumentExtension;
}

export const Document: React.SFC<IProps> = ({extension}) => {
  const labelHeight = 24;
  const labelWidth = 50;

  return (
    <svg viewBox="0 0 61 73" className={cn("document-icon", styles.document, extension)}>
      ...
      <text className={styles.text} fontSize="8">
        <tspan x="12" y="47">.{extension}</tspan>
      </text>
    </svg>
  )
}
