...
run_backend() {
    ...
    if [[ -z "${BACKEND_DEPLOYMENT_KEY}" ]]; then
        ...
        if [ ! -d "./platform-backend" ]; then
            ...
            chmod 600 ./cert
            ssh-agent sh -c 'ssh-add ./cert; git clone git@github.com:Neufund/platform-backend.git; cd ./platform-backend; git reset --hard 27af7501d018b3da60101e051a45ab19c490cbc1'
        fi
    fi
