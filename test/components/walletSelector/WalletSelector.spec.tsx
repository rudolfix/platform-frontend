...
describe("<WalletSelector />", () => {
  it("should render all three wallet tabs", () => {
    ...
    expect(component.find({ "data-test-id": "wallet-selector-ledger" }).length).to.be.eq(1);
    expect(component.find({ "data-test-id": "wallet-selector-browser" }).length).to.be.eq(1);
    expect(component.find({ "data-test-id": "wallet-selector-light" }).length).to.be.eq(1);
  });
  ...
  it("should fire correct actions for selected wallets", () => {
    ...
    walletInBrowserSelected.find({ "data-test-id": "wallet-selector-browser" }).simulate("select");
    ...
    ledgerSelected.find({ "data-test-id": "wallet-selector-ledger" }).simulate("select");
    ...
    lightWalletSelected.find({ "data-test-id": "wallet-selector-light" }).simulate("select");
